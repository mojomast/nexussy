...
- MCP tools: `nexussy_start_pipeline`, `nexussy_get_status`, `nexussy_list_sessions`, `nexussy_get_artifacts`, `nexussy_interview_answer`, `nexussy_pause`, `nexussy_resume`, `nexussy_cancel`, `nexussy_inject`, `nexussy_steer`, `nexussy_worker_spawn`, `nexussy_worker_assign`, and `nexussy_list_workers`.
- Steering: `nexussy_steer` accepts `{target: orchestrator|worker, run_id, worker_id?, message, priority}`, persists every event to the `steer_events` SQLite table, and either appends to `engine.steer_queue[run_id]` (drained into `engine.steer_context[run_id]` at each stage boundary) or forwards to the worker inject path.
- Task slicing: develop stage `_slice_devplan_tasks(devplan_text)` parses the devplan artifact into atomic specs (`id`, `title`, `acceptance_criteria`, `files_allowed`); each worker receives one spec as `json.dumps(task_spec)` in its Pi RPC `request` payload.
- Interview auto-skip: setting `metadata.skip_interview = "true"` on `/pipeline/start` synthesizes all interview answers from the project description (each marked `source="auto"`) and bypasses the human pause gate.
- Merge conflict recovery: `merge_single_worker` now writes a `conflict_report` artifact, attempts `git checkout --ours` + `git add` + `git commit --no-edit` per conflicting path, and only raises if recovery fails.
