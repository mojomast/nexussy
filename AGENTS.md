...
- Task slicing: plan saves both `devplan.md` and a `devplan_tasks` JSON sidecar; develop stage `_slice_devplan_tasks(devplan_text, devplan_tasks_json=None)` reads the sidecar first and falls back to markdown parsing into atomic specs (`id`, `title`, `acceptance_criteria`, `files_allowed`). Each worker receives one spec as `json.dumps(task_spec)` in its Pi RPC `request` payload.
