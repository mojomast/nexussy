...
- TUI manual interview: `/new <description>` defaults to human interview mode. The TUI hydrates `interview` artifacts, plain composer text answers pending questions, suggested answers/evidence can be displayed, answers are submitted through `/pipeline/{session_id}/interview/answer`, and SSE resumes with `Last-Event-ID` plus event-id dedupe to avoid replay loops. Use `/new --auto-interview <description>` or `/new --skip-interview <description>` for auto-answer behavior.
