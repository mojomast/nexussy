...
- [x] **Residual follow-ups closed** — The residual-closure pass completed the deferred cleanup set without changing ownership boundaries: `ToolDisplay.json` and `StagesConfig.validate` now use non-shadowing internal names with compatibility aliases, WorkerID/TaskID/EventID validation and ULID event generation are covered, OpenTUI/PiTUI renderer harness tests were added, and web anchor constants now load from a shared web-owned asset. Verification passed with `python3 -m pytest -q core/tests` (159), `cd tui && bun test` (75), `cd tui && bun run typecheck`, `python3 -m pytest -q web/tests` (55), shell syntax, ops tests, installer dry-run, and core/web startup smokes.
