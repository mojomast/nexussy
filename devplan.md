...
- 2026-05-15: README product promise was tightened around the idea-to-built-project workflow, clarifying that completed code in the worktree is the primary deliverable, artifacts are the trace, and `scripts/smoke_integration.sh` is the strongest local proof path while output quality remains provider/executor/test dependent.
- 2026-05-15: CLI launch UX improved: normal `./nexussy.sh cli` startup now prompts for provider/model setup when no API key is configured, Ask-mode questions use provider-backed `/assistant/reply` when configured, build-looking plain text presents a safe confirmation path to `/new`, onboarding points first-time users to setup/doctor, and the side rail labels ask versus active pipeline mode. TUI tests and typecheck pass.
- 2026-05-15: README was polished for GitHub/new-user onboarding with a top-level quick start, first-run provider setup transcript, ask-versus-build examples, deliverable/artifact locations, common usage table, and cleaner install details.
- 2026-06-04: AgentRouter full-pipeline hardening completed. Core and bundled worker AgentRouter calls present as Claude Code; develop now fails worker JSON-RPC errors and no-change workers, passes all devplan tasks to a single worker when needed, includes accumulated steering in worker specs, and has regression coverage. Live AgentRouter proof run `e0dc5296-11d3-4242-b813-70d214a22997` built `wordcount-tool` with changed files `wordcount.py`, `tests/test_wordcount.py`, and `README.md`; pytest and CLI checks passed.
- 2026-06-04: Pipeline-aware TUI and dynamic manual interview pass completed. OpenTUI now exposes all stages including optional `validate_browser`, per-stage controls, stage chat, model routing/profile overlays, AgentRouter route overrides on `/new`, direct core autostart with repo `.env` provider discovery, redacted provider-error rendering, manual interview as the default `/new` path, interview artifact hydration/answer submission, and SSE resume/dedupe. Core interview generation is graph-aware, parses noisy JSON, and supports suggested answers/evidence.
- 2026-06-04: Adaptive interview and live pipeline UX fix completed. Manual interview now asks one question at a time and adapts follow-up questions to prior answers; answer submission no longer unpauses core until the interview loop is complete. OpenTUI/PiTUI stream runs in the background with duplicate stream guards, refocuses the composer, hydrates every interview artifact update, and shows running stage/worker activity instead of appearing frozen after interview answers.
<!-- PROGRESS_LOG_END -->
