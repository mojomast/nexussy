...
test("live-core mode preserves auth headers across HTTP routes used by UI", async () => {
  ...
  await client.health(); await client.status("r"); await client.spawn({run_id:"r", role:"backend", task:"Do it"}); await client.fileLocks("r"); await client.events("r");
  expect(client.mode).toBe("live-core");
  expect(seen.map(s => new URL(s.url).pathname)).toEqual(["/health","/pipeline/status","/swarm/spawn","/swarm/file-locks","/events"]);
