...
import { renderStageBar, renderStageBarFooter } from "./components/StageBar";
...
export function renderPanels(state:TuiState): RenderedPanels {
  const footer = renderStageBarFooter(state);
  const left = [`Agents (${Object.keys(state.workers).length})`, ...Object.values(state.workers).map(w=>`${w.role} ${w.worker_id} ${w.status}${w.task_title?` — ${w.task_title}`:""}`), "", "Stages", renderStageBar(state), ...STAGES.map(x=>`${x}: ${state.stages[x]}`), "", `Paused: ${state.paused ? "yes" : "no"}`, state.finalStatus ? `Final: ${state.finalStatus}` : ""].filter(Boolean).join("\n");
  const center = ["Stream", state.handoffPrompt ? `! handoff: ${state.handoffPrompt.trigger}${state.handoffPrompt.blocking ? " (blocking)" : ""}` : "", state.connectionError ? `! connection: ${state.connectionError}` : "", state.authError ? `! auth: ${state.authError}` : "", ...state.logs.map(r => `${r.kind === "tool" ? (r.collapsed ? "▸" : "▾") : "•"} ${r.title}${r.collapsed ? "" : `\n${r.text}`}`), "", "Input: describe a project, /new DESCRIPTION, /handoff, /secrets, /spawn, /inject, /pause, /resume, /stage, /export, /quit"].filter(Boolean).join("\n");
  const activeBlockers = Object.values(state.blockers).filter(b=>!b.resolved);
  const secretRows = state.secrets.length ? state.secrets.map(s=>`${s.configured ? "configured" : "missing"} ${s.name}${s.configured ? ` (${s.source})` : ""}`) : ["run /secrets to check provider keys"];
  const right = ["DevPlan", ...(state.devplan.length?state.devplan:["(no devplan updates)"]), "", "Provider Keys", ...secretRows, state.secretNotice ?? "", "", `Tokens: ${footer.text} [${footer.color}]  Cost: $${state.usage.cost_usd.toFixed(4)}`, "", "Files", ...state.locks.map(l=>`${l.status} ${l.path} by ${l.worker_id}`), "", "Git", ...state.gitEvents.map(g=>`${g.action}: ${g.message}`), "", "Artifacts", ...state.artifacts.map(a=>`${a.kind}: ${a.path}`), "", "Blockers", ...(activeBlockers.length?activeBlockers.map(b=>`${b.severity}: ${b.message}`):["(none)"])].filter(Boolean).join("\n");
