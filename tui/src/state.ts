...
import { defaultRoutingForStages, discoverModelOptions, routingProfileGateStages, setStageRoutingModel, switchRoutingProfile } from "./lib/routing";
...
export interface TuiConfig { contextWindowSize?:number; model?:string; projectName?:string; gateStages?:boolean; }
...
export const STAGES: StageName[] = ["interview","design","validate","validate_browser","plan","review","develop"];
...
export function createState(config:TuiConfig = {}): TuiState { const normalized={ gateStages:true, ...config }; const usage={...zeroUsage, model:normalized.model}; const modelOptions=discoverModelOptions([]); return { stages:Object.fromEntries(STAGES.map(s=>[s,"pending"])) as Record<StageName,StageRunStatus>, checkpoints:{}, workers:{}, locks:[], logs:[], devplan:[], usage, contextBudget:budgetFor(usage, normalized), config:normalized, paused:false, gitEvents:[], blockers:{}, artifacts:[], secrets:[], modelOptions, routing:defaultRoutingForStages(STAGES, modelOptions), routingProfile:"default", stageControlNotes:[] }; }
...
export function reduceRoutingProfile(state:TuiState, profile:RoutingProfileName): TuiState { return { ...state, routingProfile:profile, routing:switchRoutingProfile(state.routing, profile), config:{ ...state.config, gateStages:routingProfileGateStages[profile] } }; }
...
export function reduceEvent(state:TuiState, env:EventEnvelope): TuiState {
  ...
  else if (env.type === "pause_state_changed") { const stage=(p.stage ?? activeStage(state)) as StageName|undefined; s.paused=Boolean(p.paused); if(stage) s.stages[stage]=s.paused ? "paused" : "running"; if(s.paused) s.handoffPrompt={trigger:"paused",blocking:false}; }
...
function activeStage(state:TuiState): StageName|undefined { return Object.entries(state.stages).find(([, status]) => status === "running" || status === "paused")?.[0] as StageName|undefined; }
