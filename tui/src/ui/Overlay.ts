...
export function renderOverlay(state: ChatUiState): string[] {
  ...
  if (state.overlay === "pipeline") return ["Pipeline", `selected: ${state.selectedStage ?? "none"}`, "Stage      Status               Primary -> Fallback Worker", ...renderPipelineRows(state.app, state.pendingGate, state.pendingInterview).map((row, index) => `${STAGES[index] === state.selectedStage ? ">" : " "} ${row}`), "", "Focused controls: arrows/1-7 select stage; P pause, R resume, X cancel, C chat", "Slash: /pause <stage>, /resume <stage>, /cancel <stage>, /stage-chat <stage>"];
