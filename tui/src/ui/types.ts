...
export interface ChatUiState {
  ...
  dataPanel?: { title:string; lines:string[] };
}
...
export type ClientLike = {
  ...
  cancel?(run_id:string, reason:string): Promise<unknown>|unknown;
  secrets(): Promise<unknown>|unknown;
  memory?(session_id?:string): Promise<unknown>|unknown;
  graph?(session_id?:string, run_id?:string): Promise<unknown>|unknown;
  config?(): Promise<unknown>|unknown;
  events?(run_id:string, after_sequence?:number, limit?:number): Promise<unknown>|unknown;
