...
async def release_file(db, run_id: str, path: str, worker_id: str, emit=None) -> FileLock:
    ...
    lock=FileLock(path=rel,worker_id=worker_id,run_id=run_id,status=LockStatus.released)
    if emit: await emit("file_released", lock)
    return lock
