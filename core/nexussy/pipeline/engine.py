...
from nexussy.providers import (
    active_rate_limit,
    complete,
    effective_secret_env,
    mock_requested,
    model_available,
    provider_error_for_model,
    provider_for_model,
    select_stage_model,
)
...
def _rate_limited_error(provider: str, model: str, limited) -> ErrorResponse:
    return ErrorResponse(
        ...
        details={"provider": provider, "model": model, "reset_at": limited["reset_at"]},
        retryable=True,
    )
...
class Engine:
    ...
    def __init__(self, db, config):
        ...
        self.tasks = {}
        self.paused = {}
        self.interview_waiters: dict[str, asyncio.Future] = {}
        self.interview_questions: dict[str, list[InterviewQuestionAnswer]] = {}
        self.session_runs: dict[str, str] = {}
        self.active_worker_rpcs: dict[str, list] = {}
        self.blocked_previous_status: dict[str, str] = {}
        self.git_lock = asyncio.Lock()
        self.provider_env_cache: dict | None = None
        self._run_usage: dict[str, TokenUsage] = {}
        self.steer_queue: dict[str, list[dict]] = {}
        self.steer_context: dict[str, list[str]] = {}
    ...
    async def consume_steer(self, run_id: str) -> list[dict]:
        ...
        if msgs:
            ...
            if ids:
                await self.db.write(
                    lambda con: [
                        con.execute("UPDATE steer_events SET consumed_at=? WHERE id=?", (consumed_at, msg_id))
                        for msg_id in ids
                    ]
                )
        return msgs
    ...
    async def restore_interview_state(self) -> None:
        ...
        for row in rows:
            ...
            if cps and rid not in self.tasks:
                logger.warning(
                    "Run %s found in 'running' state on startup with no active task - marking as failed. Resubmit via POST /pipeline/start with resume_run_id=%s",
                    rid,
                    rid,
                )
                await self.db.write(
                    lambda con, r=rid: con.execute(
                        "UPDATE runs SET status='failed', finished_at=? WHERE run_id=?", (now_utc().isoformat(), r)
                    )
                )
    ...
    async def _persist_event(self, env: EventEnvelope):
        ...
        def tx(con):
            row = con.execute(
                "SELECT COALESCE(MAX(sequence),0)+1 AS sequence FROM events WHERE run_id=?", (env.run_id,)
            ).fetchone()
    ...
    async def emit(self, typ: SSEEventType, session_id: str, run_id: str, payload):
        ...
        for q in list(self.queues.get(run_id, set())):
            try:
                ...
                slow = EventEnvelope(
                    sequence=0,
                    type=SSEEventType.pipeline_error,
                    session_id=session_id,
                    run_id=run_id,
                    payload=ErrorResponse(
                        error_code=ErrorCode.sse_client_slow, message="SSE client queue exceeded", retryable=True
    ...
    async def replay(self, run_id, last_event_id=None, after_sequence=0, limit=10000):
        if last_event_id:
            rows = await self.db.read("SELECT sequence FROM events WHERE event_id=?", (last_event_id,))
            after_sequence = rows[0]["sequence"] if rows else after_sequence
        rows = await self.db.read(
            "SELECT payload_json FROM events WHERE run_id=? AND sequence>? ORDER BY sequence LIMIT ?",
    ...
    async def create_session(self, req: SessionCreateRequest | PipelineStartRequest):
        ...
        sid = str(uuid4())
        now = now_utc()
        root = pathlib.Path(self.config.projects_dir).expanduser() / slug
        main = root / "main"
        ...
        if req.existing_repo_path:
            ...
            try:
                result = subprocess.run(
                    ["git", "-C", str(src), "rev-parse", "--git-dir"], capture_output=True, text=True
                )
        ...
        detail = SessionDetail(
            session=SessionSummary(
                session_id=sid, project_name=req.project_name, project_slug=slug, created_at=now, updated_at=now
            ),
        ...
        await self.db.write(
            lambda con: con.execute(
                ...
                (sid, slug, req.project_name, "created", now.isoformat(), now.isoformat(), detail.model_dump_json()),
            )
        )
        return detail
    ...
    async def start(self, req: PipelineStartRequest):
        ...
        selected = {
            st.value: select_stage_model(
                self.config,
                st.value,
                {(k.value if hasattr(k, "value") else k): v for k, v in req.model_overrides.items()},
            )
            for st in STAGES
        }
        fallbacks = []
        for st, model in selected.items():
            ...
            if not model_available(model, allow_mock=allow_mock):
                if self.config.providers.allow_fallback and model_available(
                    self.config.providers.default_model, allow_mock=allow_mock
                ):
                    fallbacks.append(
                        {"stage": st, "requested_model": model, "fallback_model": self.config.providers.default_model}
                    )
        ...
        if req.resume_run_id and await resume_from_checkpoint(self.db, req.resume_run_id, req.start_stage):
            ...
            if checkpoint:
                ...
                if not (
                    stage_value == StageName.interview.value and checkpoint.path.endswith("interview-questions.json")
                ):
        ...
        run = RunSummary(
            ...
            current_stage=effective_req.start_stage,
            started_at=now,
        )
        await self.db.write(
            lambda con: (
                con.execute(
                    ...
                    (
                        ...
                        "running",
                        effective_req.start_stage.value,
                        now.isoformat(),
                        None,
                ...
                con.execute(
                    "UPDATE sessions SET status=?, updated_at=?, detail_json=? WHERE session_id=?",
                    ("running", now.isoformat(), detail.model_dump_json(), detail.session.session_id),
                ),
            )
        )
        for st in STAGES:
            await self.db.write(
                lambda con, st=st: con.execute(
                    "INSERT OR REPLACE INTO stage_runs VALUES(?,?,?,?,?,?,?)",
                    (rid, st.value, "pending", 0, None, None, None),
                )
            )
        ...
        for fb in fallbacks:
            await self.emit(
                ...
                ErrorResponse(
                    ...
                    message="provider/model unavailable; using fallback model",
                    details=fb,
                    retryable=True,
                ),
            )
        ...
        return RunStartResponse(
            session_id=detail.session.session_id,
            run_id=rid,
            stream_url=f"/pipeline/runs/{rid}/stream",
            status_url=f"/pipeline/status?run_id={rid}",
        )
        ...
        try:
            ...
            while i <= stop:
                ...
                attempt = (
                    ...
                    else ((review_iterations + 1) if st == StageName.review else 1)
                )
                t0 = now_utc()
                await self._update_stage_status(run.run_id, st, "running", attempt=attempt, started_at=t0)
                await self.emit(
                    SSEEventType.stage_transition,
                    detail.session.session_id,
                    run.run_id,
                    StageTransitionPayload(
                        from_stage=prev,
                        to_stage=st,
                        from_status=StageRunStatus.passed if prev else None,
                        to_status=StageRunStatus.running,
                        reason="stage started",
                    ),
                )
                made = await self._artifacts_for_stage(
                    st, req, detail, run.run_id, cp, root, selected_models or {}, allow_mock, review_feedback_for_plan
                )
                artifacts.extend(made)
                if st == StageName.validate:
                    validation_iterations += 1
                    fail_for = int(
                        req.metadata.get("validate_fail_iterations", 0)
                        or (
                            self.config.stages.validate.max_iterations or 3
                            if req.metadata.get("force_validate_fail")
                            else 0
                        )
                    )
                    ...
                    if validation_iterations <= fail_for or not validation_passed:
                        await self._update_stage_status(
                            ...
                            StageName.validate,
                            "retrying",
                            attempt=validation_iterations,
                            error=ErrorResponse(
                                error_code=ErrorCode.validation_error,
                                message="validation issues require design correction",
                                retryable=True,
                            ),
                        )
                        if validation_iterations < (self.config.stages.validate.max_iterations or 3):
                            await self.emit(
                                ...
                                run.run_id,
                                StageTransitionPayload(
                                    from_stage=StageName.validate,
                                    to_stage=StageName.design,
                                    from_status=StageRunStatus.failed,
                                    to_status=StageRunStatus.retrying,
                                    reason="validation corrections returned to design",
                                ),
                            )
                            i = STAGES.index(StageName.design)
                            prev = StageName.validate
                            continue
                        raise RuntimeError("validate max iterations exceeded")
                if st == StageName.review and (
                    req.metadata.get("force_review_fail")
                    or not await self._latest_report_passed(run.run_id, "review_report", True)
                ):
                    review_iterations += 1
                    ...
                    await self._update_stage_status(
                        ...
                        StageName.review,
                        "retrying",
                        attempt=review_iterations,
                        error=ErrorResponse(
                            error_code=ErrorCode.stage_failed,
                            message="review feedback requires plan correction",
                            retryable=True,
                        ),
                    )
                    if review_iterations < int(
                        req.metadata.get("review_fail_iterations", self.config.stages.review.max_iterations or 2)
                    ):
                        await self.emit(
                            SSEEventType.stage_transition,
                            detail.session.session_id,
                            run.run_id,
                            StageTransitionPayload(
                                from_stage=StageName.review,
                                to_stage=StageName.plan,
                                from_status=StageRunStatus.failed,
                                to_status=StageRunStatus.retrying,
                                reason="review feedback returned to plan",
                            ),
                        )
                        i = STAGES.index(StageName.plan)
                        prev = StageName.review
                        continue
                    raise RuntimeError("review max iterations exceeded")
                fin = now_utc()
                await self._update_stage_status(run.run_id, st, "passed", finished_at=fin)
                await self.emit(
                    SSEEventType.stage_status,
                    detail.session.session_id,
                    run.run_id,
                    StageStatusSchema(
                        stage=st, status=StageRunStatus.passed, attempt=1, finished_at=fin, output_artifacts=made
                    ),
                )
                ck = await save_checkpoint(
                    ...
                    run.run_id,
                    st,
                    f".nexussy/checkpoints/{st.value}.json",
                    content=await self._checkpoint_content_for_stage(run.run_id, st, made),
                )
                await self.emit(SSEEventType.checkpoint_saved, detail.session.session_id, run.run_id, ck)
                prev = st
                i += 1
                if self.steer_queue.get(run.run_id):
                    await self.consume_steer(run.run_id)
                if st == StageName.plan:
                    review_feedback_for_plan = ""
            if self.config.stages.validate_browser.enabled and prev == StageName.develop:
                browser_refs = await self._run_validate_browser(
                    req, detail, run.run_id, cp, root, selected_models or {}, allow_mock, prev
                )
            ...
            await self.db.write(
                lambda con: con.execute(
                    "UPDATE runs SET status=?, finished_at=?, current_stage=?, usage_json=? WHERE run_id=?",
                    (
                        "passed",
                        now_utc().isoformat(),
            ...
            await self.emit(
                ...
                run.run_id,
                DonePayload(
                    final_status=RunStatus.passed, summary="pipeline completed", artifacts=artifacts, usage=usage
                ),
            )
            ...
            await self.db.write(
                lambda con: con.execute(
                    "UPDATE runs SET status=?, finished_at=?, usage_json=? WHERE run_id=?",
                    ("cancelled", now_utc().isoformat(), usage.model_dump_json(), run.run_id),
                )
            )
            ...
            await self.db.write(
                lambda con: con.execute(
                    "UPDATE runs SET status=?, finished_at=?, usage_json=? WHERE run_id=?",
                    ("failed", now_utc().isoformat(), usage.model_dump_json(), run.run_id),
                )
            )
            ...
            await self.emit(
                ...
                run.run_id,
                DonePayload(
                    final_status=RunStatus.failed, summary="pipeline failed", artifacts=artifacts, usage=usage, error=er
                ),
            )

    async def _save_art(self, run_id, session_id, root, kind, text, phase=None):
        rel = artifact_path(kind, phase)
        meta = safe_write(root, rel, text)
        ref = ArtifactRef(kind=kind, path=rel, sha256=meta["sha256"], bytes=meta["bytes"], phase_number=phase)
        await self.emit(
            SSEEventType.artifact_updated, session_id, run_id, ArtifactUpdatedPayload(artifact=ref, action="created")
        )
        await self.db.write(
            lambda con: con.execute(
                "INSERT OR REPLACE INTO artifacts VALUES(?,?,?,?,?,?,?,?)",
                (run_id, kind, rel, ref.sha256, ref.bytes, ref.updated_at.isoformat(), text, phase),
            )
        )
        return ref
    ...
    async def _ensure_stage_run(self, run_id: str, stage: StageName):
        await self.db.write(
            lambda con: con.execute(
                ...
                (run_id, stage.value, "pending", 0, None, None, None),
            )
        )
    ...
    async def _run_validate_browser(self, req, detail, rid, cp, root, selected_models, allow_mock, prev):
        ...
        t0 = now_utc()
        await self._update_stage_status(rid, st, "running", attempt=1, started_at=t0)
        await self.emit(
            SSEEventType.stage_transition,
            detail.session.session_id,
            rid,
            StageTransitionPayload(
                from_stage=prev,
                to_stage=st,
                from_status=StageRunStatus.passed,
                to_status=StageRunStatus.running,
                reason="stage started",
            ),
        )
        ...
        try:
            ...
            skipped = False
        fin = now_utc()
        status = StageRunStatus.skipped if skipped else StageRunStatus.passed
        if not passed and not skipped:
            error = ErrorResponse(
                error_code=ErrorCode.stage_failed, message="browser validation failed", retryable=False
            )
        ...
        await self.emit(
            ...
            detail.session.session_id,
            rid,
            StageStatusSchema(stage=st, status=status, attempt=1, finished_at=fin, output_artifacts=made),
        )
        return made

    async def execute_worker_tool(
        self, run_id: str, worker_id: str, tool: ToolName, arguments: dict[str, JsonValue] | None = None
    ):
        ...
        await self.emit(
            ...
            run_id,
            ToolCallPayload(
                call_id=call_id, stage=stage, tool_name=tool_value, arguments=arguments, worker_id=worker_id
            ),
        )
        try:
            ...
            payload = ToolOutputPayload(
                ...
                success=True,
                result_text=text,
                display=ToolDisplay(kind="json", json=result),
                worker_id=worker_id,
            )
    ...
    async def _execute_local_worker_tool(
        self, worker: Worker, tool: ToolName, arguments: dict[str, JsonValue], sid: str, run_id: str, call_id: str
    ) -> dict[str, JsonValue]:
        ...
        if name == ToolName.read_file.value:
            ...
            return {
                "path": str(path.relative_to(pathlib.Path(worker.worktree_path).resolve(strict=False))),
        ...
        if name == ToolName.search_code.value:
            ...
            for p in root.rglob("*"):
                if len(matches) >= limit:
                    break
                if not p.is_file() or any(part in {".git", ".nexussy"} for part in p.parts):
                    continue
                try:
        ...
        if name == ToolName.bash.value:
            ...
            proc = await asyncio.create_subprocess_exec(
                *argv,
                ...
                env={
                    "HOME": worker.worktree_path,
                    "PATH": "/usr/local/bin:/usr/bin:/bin",
                    "SHELL": "/bin/sh",
                    "TERM": "dumb",
                    "LANG": os.environ.get("LANG", "en_US.UTF-8"),
                    "NEXUSSY_WORKTREE": worker.worktree_path,
                },
                start_new_session=True,
            )
            try:
                out, err = await asyncio.wait_for(
                    proc.communicate(), timeout=min(float(arguments.get("timeout_s") or 30), 120.0)
                )
            ...
            return {
                ...
                "stdout": scrub_log(out[:cap].decode(errors="replace")),
                "stderr": scrub_log(err[:cap].decode(errors="replace")),
                "truncated": len(out) > cap or len(err) > cap,
            }
        raise ValueError(f"Unknown tool: {name}")

    async def _provider_text(
        self, st: StageName, sid: str, rid: str, prompt: str, selected_models, allow_mock: bool
    ) -> str:
        ...
        max_retries = max(
            1,
            int(
                getattr(stage_cfg, "max_retries", self.config.providers.max_retries)
                or self.config.providers.max_retries
                or 1
            ),
        )
        last_error = None
        for attempt in range(max_retries):
            try:
                ...
                result = await complete(
                    ...
                    timeout_s=self.config.providers.request_timeout_s,
                    _env=self.provider_env_cache,
                    db=self.db,
                )
                break
            except Exception as e:
                        ...
                        raise ProviderStartError(
                            ErrorResponse(
                                ...
                                details={
                                    "provider": provider_for_model(model),
                                    "model": model,
                                    "reset_at": (now_utc() + timedelta(seconds=60)).isoformat(),
                                },
                                retryable=True,
                            )
                        ) from e
    ...
    async def _latest_artifact_text(self, run_id: str, kind: str) -> str:
        rows = await self.db.read(
            "SELECT content_text FROM artifacts WHERE run_id=? AND kind=? ORDER BY updated_at DESC LIMIT 1",
            (run_id, kind),
        )
    ...
    async def _latest_interview_artifact(self, run_id: str) -> InterviewArtifact | None:
        rows = await self.db.read(
            "SELECT content_text FROM artifacts WHERE run_id=? AND kind='interview' ORDER BY updated_at DESC LIMIT 1",
            (run_id,),
        )
    ...
    async def submit_interview_answers(self, session_id: str, answers: dict[str, str]) -> InterviewArtifact:
        ...
        if not rid:
            rows = await self.db.read(
                "SELECT run_id FROM runs WHERE session_id=? ORDER BY started_at DESC LIMIT 1", (session_id,)
            )
            rid = rows[0]["run_id"] if rows else None
        if not rid:
            raise KeyError("run")
        current = None
        questions = self.interview_questions.get(session_id)
        if not questions:
            current = await self._latest_interview_artifact(rid)
        ...
        answered = [
            InterviewQuestionAnswer(
                question_id=q.question_id, question=q.question, answer=answers[q.question_id].strip(), source="user"
            )
            for q in questions
        ]
    ...
    async def _artifacts_for_stage(
        self, st, req, detail, rid, cp, root, selected_models=None, allow_mock=False, review_feedback_for_plan: str = ""
    ):
        ...
        if handler:
            return await handler(
                ...
                selected_models or {},
                allow_mock,
                review_feedback_for_plan=review_feedback_for_plan,
            )
        return []
        ...
        await self.db.write(
            lambda con: con.execute(
                ...
                (
                    ...
                    w.branch_name,
                    w.pid,
                    w.usage.model_dump_json(),
                    w.last_error.model_dump_json() if w.last_error else None,
                    w.model_dump_json(),
                ),
            )
        )
    ...
    async def _persist_worker_task(
        self, run_id: str, worker_id: str, task_id: str, phase_number: int | None, title: str, status: WorkerTaskStatus
    ):
        now = now_utc().isoformat()
        status_value = status.value if hasattr(status, "value") else status
        await self.db.write(
            lambda con: con.execute(
                "INSERT OR REPLACE INTO worker_tasks VALUES(?,?,?,?,?,?,COALESCE((SELECT created_at FROM worker_tasks WHERE task_id=?),?),?)",
                (task_id, run_id, worker_id, phase_number, title, status_value, task_id, now, now),
            )
        )
